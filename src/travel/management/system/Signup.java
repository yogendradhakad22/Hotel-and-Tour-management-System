...
public class Signup extends JFrame implements ActionListener{
	...
	contentPane.add(lblSecurityQuestion);
        ...
        comboBox = new JComboBox(); // dropdown for choises
	...
	contentPane.add(lblAnswer);
        ...
        Image i1 = c1.getImage().getScaledInstance(200, 200,Image.SCALE_DEFAULT); // to customize the image
        ImageIcon i2 = new ImageIcon(i1); // convert in icon bcs JLabel access only icon
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            ...
            if(ae.getSource() == b1){  // which button is called 
                String sql = "insert into account(username, name, password, security, answer) values(?, ?, ?, ?, ?)"; // insert int account table 
		PreparedStatement st = con.c.prepareStatement(sql);

		st.setString(1, textField.getText()); // for username
                st.setString(2, textField_1.getText()); // for name
		st.setString(3, textField_2.getText()); // for password
		st.setString(4, (String) comboBox.getSelectedItem()); //for Question
		st.setString(5, textField_3.getText()); // for answer
		...
		textField_3.setText("");
            ...
            if(ae.getSource() == b2){
                this.setVisible(false); // off singup 
                new Login().setVisible(true); //open  Login  when b2 = back button is clicked
